...
lazy_static! {
    static ref MQTT_CLIENT: RwLock<Option<Client>> = RwLock::new(None);
}
...
impl MqttClient {
        ...
        rt.block_on(async {
            if let Some(client) = &*MQTT_CLIENT.read() {
                ...
                let resp = client
                    .clone()
                    .subscribe(
                        vec![Topic::try_from(topic_string.to_string())?],
                        move |event| {
                                ...
                                MqttPayload::Block(block) => {
                                    ...
                                    if let Some(Payload::TaggedData(payload)) = block.payload() {
                                        info!(
                                            ...
                                            payload.tag().to_vec() //String::from_utf8(payload.tag().to_vec()).expect("found invalid UTF-8")
                                        );
    ...
    pub fn publish_message(
        ...
        let full_message =
            domain + "@@@" + &tag + "@@@" + &user_id + "@@@" + &user + "@@@" + &message;
        ...
        let tagged_data_payload =
            TaggedDataPayload::new(tag_as_bytes_vec, full_message.as_bytes().to_vec())?;
